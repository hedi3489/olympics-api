...
class VenuesService
{
    ...
    public function CreateVenues($request, $data) : Result
    {
        ...
        $v->rule('regex', 'venue_name', '/^[a-zA-Z0-9]+$/')->message('Venue name must contain only letters, digits, and spaces');
        $v->rule('regex', 'address', '/^[a-zA-Z0-9]+$/')->message('Venue address must contain only letters, digits, and spaces');
        $v->rule('regex', 'type', '/^[a-zA-Z0-9]+$/')->message('Venue type must contain only letters, digits, and spaces');
